    ...
    <div class="dashboard">
        ...
        <aside class="sidebar" :class="{ collapsed: sidebarCollapsed }">
            ...
            <nav class="sidebar-nav">
                ...
                <router-link to="/tripulacion" class="nav-item active">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="20"
                        height="20">
                        <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
                        <circle cx="9" cy="7" r="4" />
                        ...
                        <div class="modal-body">
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Nombre <span class="label-readonly" v-if="modoEdicion">— no
                                            editable</span></label>
                            ...
                            <div class="form-row" v-if="modoEdicion">
                                <div class="form-group">
                                    <label>Asignar a Vuelo adicional (opcional)</label>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group" v-if="!modoEdicion">
                                    <label>Asignar a Vuelo (opcional)</label>
                                    <div class="input-wrapper select-wrapper">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                                            width="16" height="16">
                                            <path d="M22 2L11 13" />
...
import { getTripulacion, getTripulantePorId, crearTripulante, actualizarTripulante, eliminarTripulante, asignarVuelo, getVuelos, logoutUsuario } from '../services/api'
    ...
    nextTick(() => {
        tablaInstance = window.$('#tablaTripulacion').DataTable({
            ...
            columns: [
                ...
                { data: 'nacionalidad' },
                { data: null, render: t => t.Vuelos && t.Vuelos.length > 0 ? t.Vuelos.map(v => v.codigo_vuelo).join(', ') : '—' },
                {
...
const abrirModalEditar = async (id) => {
    try {
        const res = await getTripulantePorId(id)
        const t = res.data
        form.value = {
            nombre: t.nombre, primer_apellido: t.primer_apellido,
            segundo_apellido: t.segundo_apellido || '',
            rol: t.rol, nacionalidad: t.nacionalidad, id_vuelo: ''
        }
        editandoId = id
        modoEdicion.value = true
        error.value = ''
        mostrarModal.value = true
    } catch { console.error('Error al cargar tripulante') }
}
...
const guardarTripulante = async () => {
    ...
    if (!modoEdicion.value) {
        ...
        if (!regexNombre.test(form.value.nombre.trim())) { error.value = 'El nombre debe iniciar con mayúscula, solo letras. Ejemplo: Roberto'; return }

        if (!form.value.primer_apellido.trim()) { error.value = 'El primer apellido es obligatorio.'; return }
        if (!regexNombre.test(form.value.primer_apellido.trim())) { error.value = 'El primer apellido debe iniciar con mayúscula, solo letras. Ejemplo: Mendoza'; return }

        if (form.value.segundo_apellido.trim() && !regexNombre.test(form.value.segundo_apellido.trim())) { error.value = 'El segundo apellido debe iniciar con mayúscula, solo letras.'; return }

        if (!form.value.nacionalidad.trim()) { error.value = 'La nacionalidad es obligatoria.'; return }
        if (!regexNombre.test(form.value.nacionalidad.trim())) { error.value = 'La nacionalidad debe iniciar con mayúscula, solo letras. Ejemplo: Mexicana'; return }
    }
    ...
    try {
        if (modoEdicion.value) {
            await actualizarTripulante(editandoId, form.value)
            if (form.value.id_vuelo) {
                await asignarVuelo(editandoId, { id_vuelo: form.value.id_vuelo, rol_en_vuelo: form.value.rol })
            }
            ...
            const res = await crearTripulante(form.value)
            if (form.value.id_vuelo) {
                await asignarVuelo(res.data.tripulante.id_tripulacion, { id_vuelo: form.value.id_vuelo, rol_en_vuelo: form.value.rol })
            }
        ...
        error.value = err.response?.data?.mensaje || 'Error al guardar el tripulante.'
    } finally {
        loading.value = false
    }
}
...
const confirmarEliminar = async (id) => {
    const result = await window.Swal.fire({
        ...
        text: 'Esta acción también quitará sus asignaciones a vuelos.',
    ...
    try {
        ...
        window.Swal.fire({ icon: 'error', title: 'Error', text: err.response?.data?.mensaje || 'No se pudo eliminar.', background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c' })
    }
}
