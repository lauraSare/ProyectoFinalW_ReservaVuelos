    ...
    <div class="dashboard">
        <aside class="sidebar" :class="{ collapsed: sidebarCollapsed }">
            ...
            <nav class="sidebar-nav">
                ...
                <router-link to="/vuelos" class="nav-item active">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="20"
                        height="20">
                        <path d="M22 2L11 13" />
                        ...
                        <div class="modal-body">
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Código de Vuelo <span class="label-readonly">— automático</span></label>
                                    <div class="input-wrapper input-readonly">
                                        ...
                                        <span class="readonly-text">{{ form.codigo_vuelo }}</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label>Estado <span class="label-readonly" v-if="!modoEdicion">— automático</span></label>
                                    <div v-if="modoEdicion" class="input-wrapper select-wrapper">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                                            width="16" height="16">
                                            <circle cx="12" cy="12" r="10" />
                                            <polyline points="12 6 12 12 16 14" />
                                    ...
                                    <div v-else class="input-wrapper input-readonly">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                                            width="16" height="16">
                                            <circle cx="12" cy="12" r="10" />
                                            <polyline points="12 6 12 12 16 14" />
                                        </svg>
                                        <span class="readonly-text">Programado</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-wrapper select-wrapper">
                                        ...
                                        <select v-model="form.id_avion">
                                            ...
                                            <option v-for="avion in aviones" :key="avion.id_avion"
                                                :value="avion.id_avion">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    <label>Fecha de Llegada <span class="label-auto">(calculada)</span></label>
                                    <div class="input-wrapper" :class="{ 'input-disabled': !form.fecha_llegada }">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                                            width="16" height="16">
                                            <rect x="3" y="4" width="18" height="18" rx="2" />
                                            <line x1="16" y1="2" x2="16" y2="6" />
                                        ...
                                        <svg v-if="form.fecha_llegada" viewBox="0 0 24 24" fill="none"
                                            stroke="currentColor" stroke-width="2" width="14" height="14"
                                            style="color:#7fd4a0; flex-shrink:0">
...
import { getVuelos, crearVuelo, actualizarVuelo, eliminarVuelo, getRutas, getAviones, logoutUsuario } from '../services/api'
import { obtenerCsrfToken } from '../main'
...
// ── Cálculo automático de fecha_llegada ─────────────────
watch([() => form.value.id_ruta, () => form.value.fecha_salida], () => {
    const ruta = rutas.value.find(r => r.id_ruta === form.value.id_ruta)
    if (!ruta || !form.value.fecha_salida) {
        form.value.fecha_llegada = ''
        return
    }
    const [horas, minutos, segundos] = ruta.duracion_estimada.split(':').map(Number)
    ...
    salida.setSeconds(salida.getSeconds() + (segundos || 0))
    const pad = n => String(n).padStart(2, '0')
    form.value.fecha_llegada =
        `${salida.getFullYear()}-${pad(salida.getMonth() + 1)}-${pad(salida.getDate())}` +
        `T${pad(salida.getHours())}:${pad(salida.getMinutes())}`
})
        ...
        tablaInstance = window.$('#tablaVuelos').DataTable({
            ...
            columns: [
                ...
                { data: 'codigo_vuelo' },
                {
                    ...
                    render: (d, type) => {
                        if (!d) return '—'
                        if (type === 'display') return new Date(d).toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'short' })
                ...
                },
                {
                    ...
                    render: row => {
                        if (!row.Ruta) return '—'
                        const origen = row.Ruta.Origen ? row.Ruta.Origen.codigo_iata : '?'
                        const destino = row.Ruta.Destino ? row.Ruta.Destino.codigo_iata : '?'
                        return `${origen} → ${destino} · ${row.Ruta.distancia_km} km`
                    }
                },
                {
                    data: 'Avion',
                    render: a => a ? `${a.matricula} (${a.modelo})` : '—'
                },
                {
                    data: 'id_vuelo',
                    orderable: false,
...
const cargarSelectores = async () => {
    ...
    } catch { console.error('Error al cargar rutas/aviones') }
}
...
const abrirModalCrear = () => {
    ...
    // Generar código automático QZ-001, QZ-002...
...
const guardarVuelo = async () => {
    ...
    if (!form.value.codigo_vuelo.trim()) {
        error.value = 'El código de vuelo es obligatorio.'
        return
    }
    if (!regexCodigo.test(form.value.codigo_vuelo.trim().toUpperCase())) {
        error.value = 'El código debe tener el formato XX-0000. Ejemplo: MX-1234'
        return
    }
    if (!form.value.fecha_salida) {
        error.value = 'La fecha de salida es obligatoria.'
        return
    }
    ...
    // Solo validar fecha futura al CREAR, o al EDITAR si la fecha fue modificada
    const vueloOriginal = vuelos.value.find(v => v.id_vuelo === vueloEditandoId.value)
    const fechaModificada = !modoEdicion.value || 
        (vueloOriginal && form.value.fecha_salida !== vueloOriginal.fecha_salida?.slice(0, 16))

    if (fechaModificada && new Date(form.value.fecha_salida) < manana) {
        await window.Swal.fire({
            icon: 'warning',
            title: 'Fecha no permitida',
            text: 'La fecha de salida debe ser al menos el día de mañana. No se permiten fechas pasadas ni el día de hoy.',
            background: '#1a0c10',
            color: '#f0e8e0',
            confirmButtonColor: '#c9a84c',
    ...
    if (!form.value.id_ruta) {
        error.value = 'Debes seleccionar una ruta.'
        return
    }
    if (!form.value.id_avion) {
        error.value = 'Debes seleccionar un avión.'
        return
    }
    error.value = ''
    loading.value = true
    try {
        if (modoEdicion.value) {
            ...
            window.Swal.fire({
                icon: 'success', title: '¡Actualizado!', text: 'Vuelo actualizado correctamente.',
                background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c'
            })
        } else {
            await crearVuelo(form.value)
            window.Swal.fire({
                icon: 'success', title: '¡Creado!', text: 'Vuelo creado exitosamente.',
                background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c'
            })
        }
        cerrarModal()
        ...
        error.value = err.response?.data?.mensaje || 'Error al guardar el vuelo.'
    } finally {
        loading.value = false
    }
}
...
const confirmarEliminar = async id => {
    ...
    try {
        ...
        window.Swal.fire({
            icon: 'success', title: 'Eliminado', text: 'El vuelo fue eliminado.',
            background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c'
        })
        await cargarVuelos()
    } catch (err) {
        window.Swal.fire({
            ...
            text: err.response?.data?.mensaje || 'No se pudo eliminar el vuelo.',
            background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c'
        })
    }
}
